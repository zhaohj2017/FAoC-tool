...
CTRL_OUT_BOUND = 3 # set the output bound of the controller NN: for bounded controller
...
TOL_LIE = 0.02
TOL_NORM_LIE = 0.0
TOL_BOUNDARY = 0.01 # initial boundary 0.01

WEIGHT_LIE = 1.0
WEIGHT_NORM_LIE = 0.0

HEIGHT_ASYMP = 0.05 # set the norm lower bound outside a neighborhood of the asymptotic stability point with radius 
RADIUS_ASYMP = 0.1 # set the radius of the neighborhood around the asymptotic stability point
ZERO_ASYMP = 0.001 # set the norm upper bound at the asymptotic stability point

WEIGHT_ASYMP_DOMAIN = 1.0
WEIGHT_ASYMP_POINT = 1.0
...
PLOT_EXP_V = np.array([5, 5]) # sampling from domain for plotting the vector field
PLOT_LEN_V = np.power(2, PLOT_EXP_V) # the number of samples for each dimension of domain, usually equal to PLOT_LEN_P

PLOT_EXP_P = np.array([5, 5]) # sampling from domain for plotting the scattering sampling points, could be equal to PLOT_EXP_V
